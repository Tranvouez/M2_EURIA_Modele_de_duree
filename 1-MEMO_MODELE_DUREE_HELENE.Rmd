...
Les fichiers ExposuresJapon1.csv et DeathsJapon1.csv contiennent le nombre d'individus exposés et décédés par l'âge au Japon durant la période 1947-2017, pour les âges de 0 à 110 ans, la classe d'âge 11O correspondant aux âges >= 110 (Human Mortality Database, 2019).
Définissons le taux de mortalité $q(x,t)$ et le taux instantané de mortalité $\mu(x,t)$ à l'âge $x$ et l'année $t$. Nous ferons l'approximation $q(x,t) \approx \mu(x,t)$.
Nous noterons $q_{\left(obs\right)}\left(x,t\right)$ les taux de mortalité observés calculés à partir des données pour les femmes. 

### Importation des données :

```{r}
De=read.csv("DATA/DeathsJapon1.csv",header = TRUE, sep = ";")
E=read.csv("DATA/ExposuresJapon1.csv",header = TRUE, sep = ";")

# Calibration sur l'ensemble des données : 
ex=E
de=De
```

De => la table avec les décès

E => la table avec les expositions au risque 

### Question 1 :

A partir de ces données, calibrer un \textbf{modèle de Lee-Carter} de taux instantanés de mortalité $\mu(x,t)=exp(\alpha_x\ +\ \beta_x\ *k_t)$ pour les femmes, pour les années de 1947 à 2010 et les âges de 30 à 100 ans.
...
annee=1947:2010 ; nc=length(annee)
age=30:100 ; nl=length(age)
```

Rappel : 

Défition de :
      $\hat{\mu}_(x,t)=D_(x,t)/L_(x,t)$  

tel que : 

-   D_(x,t) est le nombre de décès dans la classe d'âge x l'année t
-   L_(x,t) est l'exposition au risque dans la classe d'âge x, l'année t (avec l'approximation L_(x,t)≈E_(x,t))


```{r}
# Mettre sous forme de matrice : Homme/Femme et toute population confondu
  # Etape 1 : Calcul de mu
...
mui=matrix( de$Total[ind]/ ex$Total[ind],nl,nc)
  ...
  # Etape 2 : filtrer la population comme demandé dans l'énoncé
...
library(demography)
  ...
  # Structurer les données démographiques
Baseh=demogdata(data=muh,pop=poph,ages=age,years=annee,type="mortality",label='France',name='Hommes',lambda=1)
Basef=demogdata(data=muf,pop=popf,ages=age,years=annee,type="mortality",label='France',name='Femmes',lambda=1)
Basei=demogdata(data=mui,pop=popi,ages=age,years=annee,type="mortality",label='France',name='Individus',lambda=1)
```

Ajuster les données avec le modèle de Lee-Carter :
...
# Récupérer les taux de mortalité ajusté
predh=lch$fitted$y
predf=lcf$fitted$y  # c'est log(mu_{x,t})
predi=lci$fitted$y

# Pas demander dans le partiel de calculer les RMSE-----
...
c(rmsef,rmseh,rmsei)
# 0.1540581 0.1074303 0.1204481
```

On souhaite projeter le modèle de Lee-Carter pour les années 2011 à 2017 et les âges de 30 à 100 ans.


```{r}
# Projections 2011-2017 :-----
projh=forecast(lch,h=7)$rate$Hommes
projf=forecast(lcf,h=7)$rate$Femmes
proji=forecast(lci,h=7)$rate$Individus
```

- h = 7, le nombre d'année que l'on souhaite
=> exp(alpha_x + beta_x*h), pour les années projeter

```{r}
# Methode à la main : 
# on peut aussi les calculer à partir des kt projetés :
# ## forecast(lcf,h=7)$kt.f$mean renvoie les delta kt d'une marche aléatoire avec drift 
# il faut ajouter la valeur du dernier kt avant projection

kp=forecast(lcf,h=7)$kt.f$mean[7]+lcf$kt[length(lcf$kt)]
pf=exp(lcf$ax+lcf$bx*kp)
```

Tracer sur le même graphique les valeurs de :

-   $\hat{\mu}(x,2017)$
-   $q_{\left(obs\right)}\left(x,2017\right)$


```{r}
#lines(age,projf[,2],col='green')  # pour comparer avec données 

# Refaire les filtres pour comparer avec les années après 2011 !!
ind_aft_2011=which((de$Age>29)&(de$Age<101)&(de$Year>=2011))
annee=2011:2017 ; nc=length(annee)

muf_aft_2011 =matrix( de$Female[ind_aft_2011]/ ex$Female[ind_aft_2011],nl,nc)
muh_aft_2011 =matrix( de$Male[ind_aft_2011]/ ex$Male[ind_aft_2011],nl,nc)
mui_aft_2011 =matrix( de$Total[ind_aft_2011]/ ex$Total[ind_aft_2011],nl,nc)

# q_obs(x,2017) et mu_hat(x,2017)
plot(30:100, muf_aft_2011[,7], type = 'l', col = 'red', 
     xlab = "Âge", ylab = "Taux de mortalité", 
     main = "Taux de mortalité observés et prévus de 2011 à 2017")
lines(30:100, projf[,7], col = 'blue')
lines(30:100,pf,col='green')

# Ajout de la légende
legend("topleft",                  # Position de la légende
       legend = c("q_obs : taux de mortalité observés", 
                  "Taux instantanés de mortalité prédit",
                  "Taux instantanés calculés à la main"), 
       col = c("red", "blue","green"),      # Couleurs des courbes
       lty = c(1, 1),               # Types de lignes (1 = pleine)
       cex = 0.8)                   # Taille du texte
```

Commentaire :

Les courbes des taux instantanés de mortalités calculé à la main ou via le modèle de Lee-Carter se superpose. Cela donne donc les mêmes résultats.


Donner les valeurs de : 
-   $\hat{\mu}(90,2017)$
-   $q_{\left(obs\right)}\left(90,2017\right)$


```{r}
# comparaison q_obs(x=90,2017) et mu(x=90,2017)
colnames(muf_aft_2011)=annee
rownames(muf_aft_2011)=age
which(rownames(muf_aft_2011)==90) # 61
c(muf_aft_2011[61,7],projf[61,7])
# 0.1009978 0.1002805

pf[61]
# 0.1002805 (on retrouve le même résultat par le second calcul)
```

### Question 2 : 

Nous considérons maintenant le modèle \textbf{modèle log linéaire} :

$$
lg(x,t)=\alpha_x\ +\beta_x\ *t
$$

où $lg(x,t) = log(\frac{q(x,t)}{1-q(x,t)}$ est le logit de $q(x,t)$

Déterminer explicitement les estimateurs par moindre carrés des paramètres $\alpha_x$ et $\beta_x$ minimisant : 

$$
\sum_{x=x_{\min}}^{x_{\max}} \sum_{t=t_{\min}}^{t_{\max}} \left( \lg_{\text{obs}}(x,t) - \lg(x,t) \right)^2,
$$

où \(\lg_{\text{obs}}(x,t)\) est la valeur observée et \(\lg(x,t) = \alpha_x + \beta_x \kappa_t\) est la valeur modélisée. Nous devons estimer les paramètres \(\alpha_x\) et \(\beta_x\) de sorte que l'erreur quadratique soit minimisée.

=> Fait sur feuille

- En utilisant les formules obtenus précédemment, calibrer le modèle log linéaire pour les femmes pour les années de 1947 à 2010 et les âges de 30 à 100 ans.
...
Rappel du modèle log-linéaire, en partant des logits des $q_{(x,t)}$ : 

$$ 
logit(q_(x,t))=ln(\frac{q_{(x,t)}}{1-q_{(x,t)}})\approx ln(\mu_{(x,t)})
$$

l'approximation étant assez faible, sauf pour les âges élevés.

Estimation de $\alpha$ et $\beta$ : Utilisation de la méthode "Régression linéaire"


```{r}
# Ajuster sur le modèle log linéaire
lg=log(muf1/(1-muf1))

al=rep(0,nl)
be=rep(0,nl)

# Autre méthode, en utilisant la fonction lm pour chaque âge :
# (régression linéaire)
for (i in 1:nl) # Rappel : nl=length(age)
{reg=lm(lg[i,]~annee)
...
```{r}
# calcul des logit estimés sur 1947-2010
lgpred=lg
 for (i in 1:nl)
 {for (j in 1:nc)
...
Partons de la définition du *logit* :

$$
\text{logit}(q_{x,t}) = \ln\left(\frac{q_{x,t}}{1 - q_{x,t}}\right).
$$

1. Exponentions des deux côtés pour éliminer le logarithme :
$$
e^{\text{logit}(q_{x,t})} = \frac{q_{x,t}}{1 - q_{x,t}}.
$$

2. Multiplions chaque côté par \(1 - q_{x,t}\) :
$$
e^{\text{logit}(q_{x,t})} \cdot (1 - q_{x,t}) = q_{x,t}.
$$

3. Développons le produit à gauche :
$$
e^{\text{logit}(q_{x,t})} - e^{\text{logit}(q_{x,t})} \cdot q_{x,t} = q_{x,t}.
$$

4. Regroupons les termes en \(q_{x,t}\) :
$$
e^{\text{logit}(q_{x,t})} = q_{x,t} + e^{\text{logit}(q_{x,t})} \cdot q_{x,t}.
$$

5. Factorisons \(q_{x,t}\) à droite :
$$
e^{\text{logit}(q_{x,t})} = q_{x,t} \cdot \left(1 + e^{\text{logit}(q_{x,t})}\right).
$$

6. Isolons \(q_{x,t}\) :
$$
q_{x,t} = \frac{e^{\text{logit}(q_{x,t})}}{1 + e^{\text{logit}(q_{x,t})}}.
$$

Nous retrouvons la relation finale :
$$
q_{x,t} = \frac{\exp\left(\text{logit}(q_{x,t})\right)}{1 + \exp\left(\text{logit}(q_{x,t})\right)}.
$$
...
```{r}
## prediction 2011-2017
npa=7 # nombre d'années prédites

lgpred_aft2011=matrix(rep(0,nl*npa),nl,npa)
an=2011:2017
  for (i in 1:nl)
{for (j in 1:npa)
{lgpred_aft2011[i,j]=al[i]+be[i]*an[j]}}

qpred_aft2011=exp(lgpred_aft2011)/(1+exp(lgpred_aft2011))
dim(qpred_aft2011)
```

Représentation sur un même graphique les taux de mortalité prédit toutes méthodes confondus : 
...
plot(30:100, muf_aft_2011[,7], type = 'l', col = 'red', 
     ...
     main = "Taux de mortalité observés et prévus de 2011 à 2017")
lines(30:100, projf[,7], col = 'blue')
#lines(30:100,pf,col='green',add=T)
lines(30:100,qpred_aft2011[,7],col='cyan')

# Ajout de la légende
legend("topleft",                  # Position de la légende
       legend = c("q_obs : taux de mortalité observés", 
                  "Taux instantanés de mortalité prédit : Lee Carter",
                  #"Taux instantanés calculés à la main",
                  "Taux instantanés prédit : log linaire"), 
       col = c("red", "blue","cyan"),      # Couleurs des courbes
       lty = c(1, 1),               # Types de lignes (1 = pleine)
       cex = 0.8)                   # Taille du texte
```

Comparaison des $\hat{q}(90,2017)$ des différentes méthodes : Observé/ Lee-Carter/ Log linéaire

```{r}
# q_obs(90,2017), LC(90,2017), LogLin(90,2017)
c(muf_aft_2011[61,7],projf[61,7],qpred_aft2011[61,7])
#  0.10099780 0.10028046 0.09872513


# Calcul du RMSE E2 : fonctionne pas
# sqrt(sum((log(muf1)-log(qpred_aft2011))^2)/nl/npa)
# 0.2612912
```

# Examen 2021 : 

Examen de Mars 2021. Données Veteran. L’objectif est d’étudier les relations entre le temps de survie pour les patients atteints du cancer des poumons et les variables explicatives trt, celltype , karno , age et prior.

## Question 1:  Remplacer la variable prior par une variable binaire : 0=non, 1=oui.
...
## Question 2:  Test de comparaison des durées de survie selon celltype
...
## Question 3:  Test de comparaison des durées de survie selon trt
```{r}
...
## Question 4: Expliquer la duréee de survie en fonction des variables explicatives ci-dessus par un modèle de Cox.
...
```{r}
cox0= coxph(formula=Surv(time,status)~trt+celltype+karno+age+prior+diagtime,data=x)
summary(cox0)
```
...
```{r}
plot(survfit(cox1,newdata=veteran[1,]))
...
plot(res)
```
## Partie : Forêt aléatoire de survie : package randomForestSRC
...
```{r}
# Le code ci-dessous fonction sur R normal mais pas en Rmardown, pas trouvé pourquoi. Donc laissé en commentaire pour lancer le fichier
...
Baseh = demogdata(
  ...
  label = 'France', # label
  name = 'Hommes', # genre 
  lambda = 1 # paramètre de Box cox (Normalisation ???)
)
lch = lca(Baseh)
plot(lch)
```
